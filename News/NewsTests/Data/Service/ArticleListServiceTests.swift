...
final class ArticleListServiceTests: XCTestCase {
    func testArticleListServiceSuccess() throws {
        ...
        service.fetchArticleList(resource: MockArticleListResource(resourceName: .newsList))
            .done { list in
                articleList = list
                expectation.fulfill()
            }.catch { error in
    ...
    func testArticleListServiceFailure() throws {
        ...
        service.fetchArticleList(resource: MockArticleListResource(resourceName: .newsListNoResponse))
            .done { _ in
                expectation.fulfill()
            }.catch { error in
                theError = error
